...
int main(int argc,char* argv[])
{
    ...
    for(int k=0;k<n;k++)
    {
		...
		if(k>=average_row*rank && k<average_row*(rank+1))
		{
            ...
            A[k][k] = 1.0;
			if(rank != size-1)
			    MPI_Send(&A[k][0],n,MPI_FLOAT,rank+1,rank+1,MPI_COMM_WORLD);
		}
		else 
		{
			if(rank > k/average_row)
		    {
				MPI_Recv(&A[k][0],n,MPI_FLOAT,rank-1,rank,MPI_COMM_WORLD,&status);
	    	}
			if(rank > k/average_row && rank != size-1)
			{
				MPI_Send(&A[k][0],n,MPI_FLOAT,rank+1,rank+1,MPI_COMM_WORLD);
			}
		}
